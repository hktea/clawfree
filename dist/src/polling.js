...
const activeCleanupByAccount = new Map();
const activeStateByAccount = new Map();
const processedMessageIdsByAccount = new Map();
...
function isSessionScopedValue(value) {
    if (typeof value !== "string") {
        return false;
    }
    return value.startsWith("custom_") || value.startsWith("paid_");
}
function resolveQueueRouting(msg, fallbackSessionKey) {
...
function getProcessedMessageIds(accountId) {
...
function getOrCreateWebSocket(serverUrl, apiKey, accountId, log, clientType = "openclaw") {
...
function sendViaWebSocket(ws, to, content, originalTimestamp) {
...
function getNextPollDelay(serverUrl, apiKey, accountId, pollInterval, log) {
    getOrCreateWebSocket(serverUrl, apiKey, accountId, log, "openclaw_polling");
    return pollInterval;
}
export async function startPollingService(ctx) {
    ...
    const poll = async () => {
        ...
        if (debug && health.pollCount % 10 === 0) {
            log?.info?.(`[${account.accountId}] Polling #${health.pollCount}: offset=${offset}, consecutiveFailures=${health.consecutiveFailures}`);
        }
        ...
        try {
            ...
            log?.warn?.(`[${account.accountId}] Polling error kind=${classified.kind}, message=${classified.message}; retry in ${retryDelay}ms`);
...
export function runPollingCleanup(accountId) {
    if (accountId) {
