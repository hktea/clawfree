...
import { getPluginConfig, isConfigValid, listAccountIds as listAccountIdsFromConfig, resolveAccountConfig, validatePluginConfig, fetchConfigByToken, } from "./config.js";
// ==================== Meta 配置 ====================
const meta = {
    id: CHANNEL_ID,
    label: "ClawFree",
...
const status = {
    ...
    buildAccountSnapshot: ({ account, cfg: _cfg, runtime }) => {
        return {
            accountId: account.accountId,
            enabled: account.enabled,
            configured: isConfigValid(account.config),
...
const gateway = {
    startAccount: async (ctx) => {
        ...
        if (!account.config.apiKey?.trim() && account.config.token?.trim()) {
